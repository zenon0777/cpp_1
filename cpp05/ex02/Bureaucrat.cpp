...
/*   Updated: 2023/01/27 10:57:17 by adaifi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (F.get_sign_status() == true)
        std::cout << this->getName() << " signed " << F.get_name() << std::endl; 
    else if (this->getGrade() >= F.get_sign_grade())
        std::cout << this->getName() << " couldn't sign " << F.get_name() << "  because " <<  " grade is low " << std::endl;
    else
        std::cout << this->getName() << " couldn't sign " << F.get_name() << "  because " <<  " already signed " << std::endl;
}
